part of 'webPendingToFinish.dart';

class WebPendingToFinishTable extends StatefulWidget {
  ...
  final Null Function(TicketProgressDetails _ticketProgressDetails) onTap;

  const WebPendingToFinishTable({required this.onInit, required this.onRequestData, required this.onTap});

  @override
  _WebPrintTableState createState() => _WebPrintTableState();
}

class _WebPrintTableState extends State<WebPendingToFinishTable> {
  int _rowsPerPage = 20;
  ...
  void didChangeDependencies() {
    ...
    if (_dessertsDataSource == null) {
      _dessertsDataSource = PrintDataSourceAsync(context, onRequestData: widget.onRequestData, onTap: (TicketProgressDetails ticketProgressDetails) {
        widget.onTap(ticketProgressDetails);
      });
    }
  ...
  List<DataColumn> get _columns {
    return [
      ...
      DataColumn2(size: ColumnSize.S, label: Text('Option'), numeric: true),
    ];
  }
...
class PrintDataSourceAsync extends AsyncDataTableSource {
  ...
  Null Function(TicketProgressDetails ticketProgressDetails) onTap;
  var _selectedType;
  ...
  Future<AsyncRowsResponse> getRows(int startIndex, int count) async {
    ...
    var r = AsyncRowsResponse(
        x.totalRecords,
        x.data.map((ticketProgressDetails) {
          NsUser? nsUser = NsUser.fromId(ticketProgressDetails.finishedBy);
          return DataRow2(
              selected: false,
              onTap: () {
                onTap(ticketProgressDetails);
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                  duration: Duration(seconds: 1),
                  content: Text('Tapped on ${ticketProgressDetails.ticketId}'),
                ));
              },
              onSecondaryTap: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    duration: Duration(seconds: 1),
                    backgroundColor: Theme.of(context).errorColor,
                    content: Text('Right clicked on ${ticketProgressDetails.ticket?.id}'),
                  )),
              // specificRowHeight: this.hasRowHeightOverrides && ticketProgressDetails.fat >= 25 ? 100 : null,
              cells: [
                DataCell(Wrap(
                    direction: Axis.vertical,
                    children: [Text((ticketProgressDetails.ticket?.mo) ?? ""), Text((ticketProgressDetails.ticket?.oe) ?? "", style: TextStyle(color: Colors.red, fontSize: 12))])),
                DataCell(Text('${ticketProgressDetails.ticket?.production ?? ''}')),
                DataCell(Text('${ticketProgressDetails.finishedOn}')),
                DataCell(Wrap(
                    direction: Axis.vertical,
                    children: [Text((ticketProgressDetails.operation)), Text(("${ticketProgressDetails.operationNo}"), style: TextStyle(color: Colors.red, fontSize: 12))])),
                DataCell(IconButton(
...
class DataResponse {
  ...
  final List<TicketProgressDetails> data;
}
