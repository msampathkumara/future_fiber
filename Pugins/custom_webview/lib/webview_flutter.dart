...
/// A message that was sent by JavaScript code running in a [CustomWebView].
...
/// Android [CustomWebViewPlatform] that uses [AndroidViewSurface] to build the [CustomWebView] widget.
///
/// To use this, set [CustomWebView.platform] to an instance of this class.
///
/// This implementation uses hybrid composition to render the [CustomWebView] on
/// Android. It solves multiple issues related to accessibility and interaction
/// with the [CustomWebView] at the cost of some performance on Android versions below
/// 10. See https://github.com/flutter/flutter/wiki/Hybrid-Composition for more
/// information.
class SurfaceAndroidCustomWebView extends AndroidCustomWebView {
  @override
  Widget build({
    ...
    return PlatformViewLink(
      ...
      onCreatePlatformView: (PlatformViewCreationParams params) {
        return PlatformViewsService.initSurfaceAndroidView(
          id: params.id,
          viewType: 'plugins.flutter.io/customwebview',
          // CustomWebView content is not affected by the Android view's layout direction,
          // we explicitly set it here so that the widget doesn't require an ambient
          // directionality.
          layoutDirection: TextDirection.rtl,
          creationParams: MethodChannelCustomWebViewPlatform.creationParamsToMap(
            creationParams,
...
///
/// This is typically used in [CustomWebView.initialMediaPlaybackPolicy].
...
class JavascriptChannel {
  ...
  /// The channel's name.
  ///
  /// Passing this channel object as part of a [CustomWebView.javascriptChannels] adds a channel object to
  ...
  ///
  /// See also [CustomWebView.javascriptChannels] for more details on the channel registration mechanism.
  final String name;
...
/// the `CustomWebView` is not able to block the `CustomWebView` from receiving touch events.
/// See https://github.com/flutter/flutter/issues/53490.
class CustomWebView extends StatefulWidget {
  /// Creates a new web view.
  ///
  /// The web view can be controlled using a `CustomWebViewController` that is passed to the
  /// `onCustomWebViewCreated` callback once the web view is created.
  ///
  /// The `javascriptMode` and `autoMediaPlaybackPolicy` parameters must not be null.
  const CustomWebView({
    Key? key,
    this.onCustomWebViewCreated,
    this.initialUrl,
  ...
  /// Sets a custom [CustomWebViewPlatform].
  ///
  /// This property can be set to use a custom platform implementation for CustomWebViews.
  ///
  /// Setting `platform` doesn't affect [CustomWebView]s that were already created.
  ///
  /// The default value is [AndroidCustomWebView] on Android and [CupertinoCustomWebView] on iOS.
  static set platform(CustomWebViewPlatform? platform) {
    _platform = platform;
  }

  /// The CustomWebView platform that's used by this CustomWebView.
  ///
  /// The default value is [AndroidCustomWebView] on Android and [CupertinoCustomWebView] on iOS.
  static CustomWebViewPlatform get platform {
    if (_platform == null) {
      switch (defaultTargetPlatform) {
        case TargetPlatform.android:
          _platform = AndroidCustomWebView();
          break;
        case TargetPlatform.iOS:
          _platform = CupertinoCustomWebView();
          break;
        default:
  ...
  final CustomWebViewCreatedCallback? onCustomWebViewCreated;
  ...
  ///
  /// When a navigation is initiated by the CustomWebView (e.g when a user clicks a link)
  ...
  ///   * Setting a navigationDelegate makes the CustomWebView treat all navigations as if they were
  ...
  ///   * On Android CustomWebView versions earlier than 67(most devices running at least Android L+ should have
  ///     a later version):
  ...
  /// [CustomWebViewController.evaluateJavascript] can assume this.
  ...
  /// Invoked when a page is loading.
  final OnShowFileChooser? onShowFileChooser;
  ...
  /// Controls whether CustomWebView debugging is enabled.
  ///
  /// Setting this to true enables [CustomWebView debugging on Android](https://developers.google.com/web/tools/chrome-devtools/remote-debugging/).
  ///
  /// CustomWebView debugging is enabled by default in dev builds on iOS.
  ///
  /// To debug CustomWebViews on iOS:
  ...
  ///
  /// When the [CustomWebView] is rebuilt with a different `userAgent`, the page reloads and the request uses the new User Agent.
  ///
  /// When [CustomWebViewController.goBack] is called after changing `userAgent` the previous `userAgent` value is used until the page is reloaded.
  ///
  ...
  /// changes to this parameter are ignored (as long as the state of the [CustomWebView] is preserved).
  ///
...
class _CustomWebViewState extends State<CustomWebView> {
  final Completer<CustomWebViewController> _controller = Completer<CustomWebViewController>();

  late _PlatformCallbacksHandler _platformCallbacksHandler;

  @override
  Widget build(BuildContext context) {
    return CustomWebView.platform.build(
      context: context,
      onCustomWebViewPlatformCreated: _onCustomWebViewPlatformCreated,
      customWebViewPlatformCallbacksHandler: _platformCallbacksHandler,
  ...
  void didUpdateWidget(CustomWebView oldWidget) {
    ...
    _assertJavascriptChannelNamesAreUnique();
    _controller.future.then((CustomWebViewController controller) {
      _platformCallbacksHandler._widget = widget;
      controller._updateWidget(widget);
    });
  }

  void _onCustomWebViewPlatformCreated(CustomWebViewPlatformController? customWebViewPlatform) {
    final CustomWebViewController controller = CustomWebViewController._(widget, customWebViewPlatform!, _platformCallbacksHandler);
    _controller.complete(controller);
    if (widget.onCustomWebViewCreated != null) {
      widget.onCustomWebViewCreated!(controller);
    }
  }
...
CreationParams _creationParamsfromWidget(CustomWebView widget) {
...
WebSettings _webSettingsFromWidget(CustomWebView widget) {
  return WebSettings(
...
class _PlatformCallbacksHandler implements CustomWebViewPlatformCallbacksHandler {
  _PlatformCallbacksHandler(this._widget) {
    _updateJavascriptChannelsFromSet(_widget.javascriptChannels);
  }

  CustomWebView _widget;
...
/// Controls a [CustomWebView].
///
/// A [CustomWebViewController] instance can be obtained by setting the [CustomWebView.onCustomWebViewCreated]
/// callback for a [CustomWebView] widget.
class CustomWebViewController {
  ...
  final CustomWebViewPlatformController _customWebViewPlatformController;
  ...
  /// Accessor to the current URL that the CustomWebView is displaying.
  ///
  /// If [CustomWebView.initialUrl] was never specified, returns `null`.
  ...
  /// words, by the time this future completes, the CustomWebView may be displaying a
  /// different URL).
  Future<String?> currentUrl() {
    return _customWebViewPlatformController.currentUrl();
  }
  ...
  Future<bool> canGoBack() {
    return _customWebViewPlatformController.canGoBack();
  }
  ...
  Future<bool> canGoForward() {
    return _customWebViewPlatformController.canGoForward();
  }

  /// Goes back in the history of this CustomWebView.
  ///
  /// If there is no back history item this is a no-op.
  Future<void> goBack() {
    return _customWebViewPlatformController.goBack();
  }

  /// Goes forward in the history of this CustomWebView.
  ///
  /// If there is no forward history item this is a no-op.
  Future<void> goForward() {
    return _customWebViewPlatformController.goForward();
  }

  /// Reloads the current URL.
  Future<void> reload() {
    return _customWebViewPlatformController.reload();
  }

  /// Clears all caches used by the [CustomWebView].
  ///
  /// The following caches are cleared:
  ///	1. Browser HTTP Cache.
  ///	2. [Cache API](https://developers.google.com/web/fundamentals/instant-and-offline/web-storage/cache-api) caches.
  ///    These are not yet supported in iOS WkCustomWebView. Service workers tend to use this cache.
  ///	3. Application cache.
  ///	4. Local Storage.
  ///
  /// Note: Calling this method also triggers a reload.
  Future<void> clearCache() async {
    await _customWebViewPlatformController.clearCache();
    return reload();
  }

  Future<void> _updateWidget(CustomWebView widget) async {
    _widget = widget;
  ...
  Future<void> _updateSettings(WebSettings newSettings) {
    ...
    return _customWebViewPlatformController.updateSettings(update);
  }
  ...
  Future<void> _updateJavascriptChannels(Set<JavascriptChannel>? newChannels) async {
    ...
    if (channelsToRemove.isNotEmpty) {
      await _customWebViewPlatformController.removeJavascriptChannels(channelsToRemove);
    }
    if (channelsToAdd.isNotEmpty) {
      await _customWebViewPlatformController.addJavascriptChannels(channelsToAdd);
    }
  ...
  ///
  /// When evaluating Javascript in a [CustomWebView], it is best practice to wait for
  /// the [CustomWebView.onPageFinished] callback. This guarantees all the Javascript
  ...
  Future<String> evaluateJavascript(String javascriptString) {
    ...
    print('---------------------------_customWebViewPlatformController.evaluateJavascript(javascriptString)');
    return _customWebViewPlatformController.evaluateJavascript(javascriptString);
  }

  /// Returns the title of the currently loaded page.
  Future<String?> getTitle() {
    return _customWebViewPlatformController.getTitle();
  }

  /// Sets the CustomWebView's content scroll position.
  ///
  /// The parameters `x` and `y` specify the scroll position in CustomWebView pixels.
  Future<void> scrollTo(int x, int y) {
    return _customWebViewPlatformController.scrollTo(x, y);
  }

  /// Move the scrolled position of this view.
  ///
  /// The parameters `x` and `y` specify the amount of CustomWebView pixels to scroll by horizontally and vertically respectively.
  Future<void> scrollBy(int x, int y) {
    return _customWebViewPlatformController.scrollBy(x, y);
  }

  /// Return the horizontal scroll position, in CustomWebView pixels, of this view.
  ///
  /// Scroll position is measured from left.
  Future<int> getScrollX() {
    return _customWebViewPlatformController.getScrollX();
  }

  /// Return the vertical scroll position, in CustomWebView pixels, of this view.
  ///
  /// Scroll position is measured from top.
  Future<int> getScrollY() {
    return _customWebViewPlatformController.getScrollY();
  }
}

/// Manages cookies pertaining to all [CustomWebView]s.
class CookieManager {
  ...
  /// Clears all cookies for all [CustomWebView] instances.
  ///
  /// This is a no op on iOS version smaller than 9.
  ///
  /// Returns true if cookies were present before clearing, else false.
  Future<bool> clearCookies() => CustomWebView.platform.clearCookies();
}
